...
const GameBoard: React.FC<GameBoardProps> = ({
  ...
  const [prevAppId, setPrevAppId] = useState(currentApp.id);
  if (currentApp.id !== prevAppId) {
    setPrevAppId(currentApp.id);
    setHintStep(0);
    setCurrentScore(5);
    setFeedback(null);
    setErrorMsg(null);
  ...
  }, [currentApp.id, hintStep, isRoundOver]);
  ...
  return (
    <div className="flex flex-col md:flex-row h-full max-w-6xl mx-auto p-4 gap-6">
      <div className="w-full md:w-1/3 lg:w-1/4 h-[600px]">
        <PlayerList 
          players={players} 
